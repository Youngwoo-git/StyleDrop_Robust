...
def get_config():
    ...
    config.data_path="data/{}.json".format(style)
    config.resume_root="assets/ckpts/cc3m-285000.ckpt"
    config.adapter_path="results/{}/ckpts_I/286000.ckpt/adapter.pth".format(style)
    
    config.it_data_path = "results/{}/HF_it_data".format(style)
    config.it_phase = True
    config.stage = "II"
    if mistake_count > 0:
